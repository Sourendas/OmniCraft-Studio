  ...
  return (
    <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="rounded-3xl bg-white border border-slate-200/90 p-8 sm:p-12 space-y-5 text-slate-700 text-sm leading-relaxed">
        ...
        <p>I built it because most “free PDF” sites ask you to upload a document to a machine you do not control, then surround the download with accounts and paywalls. This project is the opposite shape: the page is a static web app. The file you pick stays in the tab as a File object. Libraries such as pdf-lib, jsPDF, Web Crypto, and the canvas API do the work on your device.</p>
        <p>JavaScript is required for the tools. If scripts are blocked, the how-to guides and policy pages still work as static HTML.</p>
        <p>FileTools Kit does not run a server that receives your documents. The website host (Vercel) still serves HTML, CSS, and JavaScript, and may log ordinary page requests (IP address, URL, user agent). Google AdSense and Vercel Analytics are separate from file bytes in the tab. Fonts may load from Google Fonts.</p>
        <h2 className="text-lg font-black text-[#0A2540]">Who operates this</h2>
        <p>I am Souren Das. I work from Bengaluru, Karnataka, India. There is no company office and no phone support line. Email is the contact method: support@filetoolskit.com for product questions, privacy@filetoolskit.com for privacy requests.</p>
        <p>The site is supported by Google AdSense Auto ads. There is no paid upgrade, no subscription, and no checkout on filetoolskit.com. Ads are how the free tools stay online. The publisher ID is pub-4409273905876536.</p>
