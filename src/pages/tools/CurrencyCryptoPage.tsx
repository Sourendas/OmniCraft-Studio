    ...
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white flex items-center gap-2.5">
            ...
            Live Currency & Remittance Arbitrage
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Real-time fiat & crypto conversion with cross-border remittance fee comparison.
          </p>
        </div>

        <div className="flex items-center gap-2 text-xs text-slate-400 font-mono">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span>Rates active: {lastUpdated}</span>
        </div>
      </div>
      ...
      <div className="my-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="rounded-3xl bg-slate-900/70 border border-slate-800 p-6 sm:p-8 backdrop-blur-xl space-y-6">
            <div>
              ...
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full pl-4 pr-24 py-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-lg font-bold text-white focus:outline-none focus:border-cyan-500"
                />
                <div className="absolute right-3">
                  <select
                    value={fromCurrency}
                    onChange={(e) => setFromCurrency(e.target.value)}
                    className="px-2 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-xs font-bold text-cyan-300 focus:outline-none"
                  >
            ...
            <div className="flex justify-center -my-2">
              <button
                ...
                className="p-2.5 rounded-full bg-slate-800 hover:bg-slate-700 text-cyan-400 border border-slate-700 transition-all hover:rotate-180 duration-300 shadow-md cursor-pointer"
              >
            ...
            <div>
              ...
              <div className="relative flex items-center">
                <div className="w-full pl-4 pr-24 py-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-emerald-400">
                  {convertedValue}
                </div>
                <div className="absolute right-3">
                  <select
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="rounded-3xl bg-slate-900/60 border border-slate-800 p-6 backdrop-blur-xl space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">
                  {fromCurrency}/{toCurrency} Price Vector
                </h3>
              </div>
                ...
                {(['1D', '7D', '1M', '1Y'] as const).map((tf) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all ${
                      selectedTimeframe === tf ? 'bg-cyan-500/20 text-cyan-300' : 'text-slate-500'
                    }`}
                  >
                    {tf}
                  </button>
            ...
            {/* SVG Trend Line */}
            <div className="w-full h-40 bg-slate-950 rounded-2xl p-3 border border-slate-800/80 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="my-8 space-y-4">
        <h3 className="text-base font-bold text-white flex items-center gap-2">
          ...
          <span>Cross-Border Remittance Cost & Speed Comparison</span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {remittanceComparison.map((r, idx) => (
            <div
              key={idx}
              className="rounded-3xl bg-slate-900/60 border border-slate-800 p-6 flex flex-col justify-between backdrop-blur-xl"
            >
              <div>
                ...
                <h4 className="text-sm font-bold text-white mb-2">{r.provider}</h4>
                <div className="space-y-2 text-xs text-slate-300">
                  <div className="flex justify-between">
                    <span className="text-slate-400">Estimated Fee:</span>
                    <span className="font-mono text-rose-400 font-bold">{r.feeAmount}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Recipient Receives:</span>
                    <span className="font-mono text-emerald-400 font-bold">{r.recipientGets}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Transfer Duration:</span>
                    <span className="text-slate-200">{r.speed}</span>
                  </div>
                </div>
              </div>
