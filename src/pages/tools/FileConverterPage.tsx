  ...
  const convertFile = async (item: ConvertFileItem) => {
    ...
    try {
      ...
      setFiles(prev =>
        prev.map(f =>
          f.id === item.id
            ? { ...f, status: 'completed', progress: 100, resultBlob: convertedBlob, resultName: outputName }
            : f
        )
      );
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#0A2540] flex items-center gap-2.5">
            <RefreshCw className="w-7 h-7 text-[#EA580C]" />
            File Converter
          </h1>
        ...
        {files.length > 0 && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#EA580C] to-[#C2410C] hover:from-[#F97316] hover:to-[#EA580C] text-white text-xs font-black shadow-lg shadow-orange-500/20 transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        {files.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-slate-500 font-bold">
              ...
              <button onClick={() => setFiles([])} className="hover:text-rose-600 transition-colors cursor-pointer">Clear Queue</button>
            </div>
            <div className="space-y-3">
              {files.map((item) => (
                <div key={item.id} className="rounded-3xl bg-white border border-slate-200/90 p-4.5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-[0_4px_20px_rgba(10,37,64,0.03)]">
                  <div className="flex items-center gap-3.5 w-full sm:w-auto">
                    <div className="p-2.5 rounded-2xl bg-[#FFEDD5] border border-[#FDBA74] shrink-0">
                      {item.type === 'image' && <ImageIcon className="w-5 h-5 text-[#C2410C]" />}
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    <div className="flex items-center gap-1.5 text-xs">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setFiles(prev => prev.map(f => (f.id === item.id ? { ...f, targetFormat: val, status: 'idle' } : f)));
                        }}
                        className="px-3 py-1.5 rounded-xl bg-[#FFF7ED] border border-slate-200 text-xs font-mono font-bold text-[#C2410C] focus:outline-none focus:border-[#EA580C]"
                      >
                    ...
                    {item.status === 'completed' && item.resultBlob && item.resultName ? (
                      <button onClick={() => downloadBlob(item.resultBlob!, item.resultName!)} className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 text-xs font-black transition-all cursor-pointer shadow-xs">
                        <Download className="w-3.5 h-3.5" /><span>Download</span>
                      </button>
                    ) : (
                      <button onClick={() => convertFile(item)} disabled={item.status === 'converting' || item.type === 'other'} className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#EA580C] hover:bg-[#F97316] text-white text-xs font-black transition-all disabled:opacity-50 cursor-pointer shadow-xs">
