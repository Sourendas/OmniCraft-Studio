...
export const PdfSuitePage: React.FC = () => {
  ...
  const loadDemoPdf = async () => {
    ...
    try {
      ...
      page1.drawText('FileTools Kit PDF', { x: 50, y: 320, size: 20, color: rgb(0, 0.64, 0.68) });
  ...
  return (
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#0A2540] flex items-center gap-2.5">
            <Layers className="w-7 h-7 text-[#00A3AD]" /> PDF Suite
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleExportAnnotatedPdf} disabled={files.length === 0 || isProcessing} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#00A3AD] to-[#008C95] text-white text-xs font-black disabled:opacity-50">
      ...
      {statusMessage && (
        <div className="my-4 p-3.5 rounded-2xl bg-[#E6F8F9] border border-[#B3EAEF] text-xs text-[#007A82] font-bold flex items-center justify-between">
      ...
      <div className="my-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-6">
          <div className="relative rounded-3xl border-2 border-dashed border-[#B3EAEF] hover:border-[#00A3AD] bg-white p-8 text-center">
            ...
            <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-[#E6F8F9] border border-[#B3EAEF] flex items-center justify-center"><Upload className="w-7 h-7 text-[#00A3AD]" /></div>
          ...
          {files.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-black text-[#007A82] uppercase tracking-wider">Loaded documents ({files.length})</h3>
                  ...
                  <div key={file.id} className="rounded-3xl bg-white border border-slate-200 p-5">
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <div className="p-2 rounded-xl bg-[#E6F8F9] border border-[#B3EAEF] text-[#007A82]"><FileText className="w-5 h-5" /></div>
                        <div>
                    ...
                    <div style={{ transform: `rotate(${file.rotation}deg)` }} className="w-full h-28 rounded-2xl bg-[#F4F8FA] border border-slate-200 flex flex-col items-center justify-center">
                      <div className="w-12 h-16 rounded-lg bg-white border border-slate-200 flex flex-col items-center justify-center text-[9px] text-slate-500 font-mono font-bold"><span>DOC</span><span className="text-[8px] text-[#007A82]">p.1-{file.pageCount}</span></div>
                    ...
                    <div className="pt-3 mt-3 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <button onClick={() => rotateFile(file.id)} className="flex items-center gap-1 text-xs font-bold text-[#007A82] px-2.5 py-1 rounded-full bg-[#E6F8F9] border border-[#B3EAEF]"><RotateCw className="w-3.5 h-3.5" /><span>Rotate +90°</span></button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="rounded-3xl bg-white border border-slate-200 p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-xs font-black text-[#0A2540] uppercase tracking-wider flex items-center gap-2"><Stamp className="w-4 h-4 text-[#00A3AD]" /><span>PDF tools</span></h3>
              <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-[#E6F8F9] text-[#007A82] font-mono font-bold border border-[#B3EAEF]">pdf-lib</span>
            </div>
            <div className="grid grid-cols-3 gap-1 p-1 rounded-2xl bg-[#F4F8FA] border border-slate-200 text-center text-xs">
              {(['watermark','metadata','split'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`py-1.5 rounded-xl font-bold ${activeTab === tab ? 'bg-white text-[#007A82]' : 'text-slate-500'}`}>{tab === 'split' ? 'Split / Range' : tab[0].toUpperCase()+tab.slice(1)}</button>
              ))}
            </div>
            {activeTab === 'watermark' && (
              <div className="space-y-4 text-xs">
                <div>
                  ...
                  <input type="text" value={watermarkText} onChange={(e) => setWatermarkText(e.target.value)} className="w-full px-3.5 py-2.5 rounded-2xl bg-[#F4F8FA] border border-slate-200" />
                </div>
                <div>
                  <div className="flex justify-between text-slate-700 font-bold mb-1"><span>Opacity</span><span className="font-mono text-[#007A82]">{Math.round(watermarkOpacity * 100)}%</span></div>
                  <input type="range" min="0.05" max="0.8" step="0.05" value={watermarkOpacity} onChange={(e) => setWatermarkOpacity(parseFloat(e.target.value))} className="w-full accent-[#00A3AD]" />
                </div>
                <div>
                  <div className="flex justify-between text-slate-700 font-bold mb-1"><span>Font size</span><span className="font-mono text-[#007A82]">{watermarkSize} pt</span></div>
                  <input type="range" min="18" max="64" step="2" value={watermarkSize} onChange={(e) => setWatermarkSize(parseInt(e.target.value))} className="w-full accent-[#00A3AD]" />
                </div>
              </div>
            )}
            {activeTab === 'metadata' && (
              <div className="space-y-3 text-xs">
                <div><label className="block text-slate-700 font-bold mb-1">Title</label><input type="text" value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} className="w-full px-3.5 py-2 rounded-xl bg-[#F4F8FA] border border-slate-200" /></div>
                <div><label className="block text-slate-700 font-bold mb-1">Author</label><input type="text" value={metaAuthor} onChange={(e) => setMetaAuthor(e.target.value)} className="w-full px-3.5 py-2 rounded-xl bg-[#F4F8FA] border border-slate-200" /></div>
                <div><label className="block text-slate-700 font-bold mb-1">Subject</label><input type="text" value={metaSubject} onChange={(e) => setMetaSubject(e.target.value)} className="w-full px-3.5 py-2 rounded-xl bg-[#F4F8FA] border border-slate-200" /></div>
                <div><label className="block text-slate-700 font-bold mb-1">Keywords</label><input type="text" value={metaKeywords} onChange={(e) => setMetaKeywords(e.target.value)} className="w-full px-3.5 py-2 rounded-xl bg-[#F4F8FA] border border-slate-200" /></div>
              </div>
            )}
            {activeTab === 'split' && (
              <div className="space-y-3 text-xs">
                <label className="block text-slate-700 font-bold">Page range (first loaded file)</label>
                <input type="text" value={splitRange} onChange={(e) => setSplitRange(e.target.value)} placeholder="e.g. 1-3, 5" className="w-full px-3.5 py-2.5 rounded-2xl bg-[#F4F8FA] border border-slate-200" />
