    ...
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#0A2540] flex items-center gap-2.5">
            <Sparkles className="w-7 h-7 text-[#00A3AD]" />
      ...
      <div className="my-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="rounded-3xl bg-white border border-slate-200/90 p-6 shadow-[0_4px_20px_rgba(10,37,64,0.03)] space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-black text-[#0A2540] uppercase tracking-wider">
                  Text Prompt
                </label>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-[11px] text-[#007A82] hover:text-[#00A3AD] font-bold flex items-center gap-1 cursor-pointer"
                  >
              ...
              <textarea
                ...
                className="w-full p-3.5 rounded-2xl bg-[#F4F8FA] border border-slate-200 text-xs text-[#0A2540] font-medium focus:outline-none focus:border-[#00A3AD] focus:bg-white leading-relaxed font-sans"
              />
            </div>

            {/* Aspect Ratio Selector */}
            <div>
              <label className="block text-xs font-black text-[#0A2540] uppercase tracking-wider mb-2">
                Aspect Ratio
              </label>
                ...
                {(Object.keys(ratioDimensions) as Array<keyof typeof ratioDimensions>).map((ratio) => (
                  <button
                    ...
                    className={`py-2 px-2 rounded-xl text-xs font-bold border transition-all cursor-pointer ${
                      selectedRatio === ratio
                        ? 'bg-[#E6F8F9] text-[#007A82] border-[#B3EAEF] shadow-xs'
                        : 'bg-[#F4F8FA] border-slate-200 text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    {ratio}
                  </button>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-2xl text-left border transition-all cursor-pointer ${
                      selectedStyle === style.name
                        ? 'bg-[#E6F8F9] text-[#007A82] border-[#B3EAEF]'
                        : 'bg-[#F4F8FA] border-slate-200 text-slate-700 hover:bg-slate-100'
                    }`}
                  >
            ...
            <div className="flex items-center justify-between text-xs pt-2">
              <span className="text-slate-500 font-mono font-bold">Seed: #{seed}</span>
              <button
                type="button"
                onClick={() => setSeed(Math.floor(Math.random() * 999999))}
                className="text-[#007A82] hover:text-[#00A3AD] font-bold text-xs flex items-center gap-1 font-mono cursor-pointer"
              >
            ...
            <button
              ...
              className="w-full py-4 px-4 rounded-full bg-gradient-to-r from-[#00A3AD] to-[#008C95] hover:from-[#00B5B8] hover:to-[#00A3AD] text-white font-black text-sm shadow-xl shadow-teal-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 active:scale-[0.99]"
            >
              {isGenerating ? (
                <>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="rounded-3xl bg-white border border-slate-200/90 p-6 shadow-[0_4px_20px_rgba(10,37,64,0.03)] flex flex-col items-center justify-center min-h-[480px] relative overflow-hidden">
            {isGenerating && (
              <div className="absolute inset-0 z-20 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6">
                <div className="w-12 h-12 rounded-full border-4 border-[#B3EAEF] border-t-[#00A3AD] animate-spin mb-4" />
                <h4 className="text-base font-black text-[#0A2540] mb-1">Rendering Visual Latents</h4>
                <p className="text-xs text-slate-500 font-mono">Flux Diffusion Pipeline • 1024px</p>
              </div>
            )}

            <div className="relative max-w-full rounded-2xl overflow-hidden shadow-md border border-slate-200 group">
              <img
              ...
              <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                <button
                  onClick={handleDownloadHighRes}
                  className="px-3.5 py-1.5 rounded-full bg-[#0A2540]/90 hover:bg-[#0A2540] text-xs font-bold text-white backdrop-blur-md flex items-center gap-1.5 cursor-pointer shadow-md"
                >
            ...
            <div className="w-full mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500 font-medium">
              <div className="flex items-center gap-2">
                <span className="font-mono font-bold text-[#007A82]">{ratioDimensions[selectedRatio].width}x{ratioDimensions[selectedRatio].height}</span>
                <span>•</span>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-[#007A82] hover:text-[#00A3AD] text-xs font-bold flex items-center gap-1 cursor-pointer"
                >
