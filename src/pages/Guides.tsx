  ...
  <div className="relative z-10 max-w-4xl mx-auto px-4 py-12">
    ...
    <div className="rounded-3xl bg-white border border-slate-200 p-8 space-y-6">
      ...
      <ul className="grid gap-4">
        {GUIDES.map((g) => (
          <li key={g.slug}>
            <Link to={`/guides/${g.slug}`} className="block p-5 rounded-2xl border border-slate-200 bg-[#FFF7ED] hover:bg-[#FFEDD5]">
              <h2 className="font-black text-[#0A2540]">{g.title}</h2>
              <p className="text-sm text-slate-600 mt-1">{g.summary}</p>
              <span className="inline-flex items-center gap-1 text-xs font-black text-[#C2410C] mt-2">Read guide <ArrowRight className="w-3.5 h-3.5" /></span>
            </Link>
          </li>
        ))}
      </ul>
