            ...
            <div key={tool.id} id={`tool-card-${tool.id}`} className="group rounded-3xl bg-white border border-slate-200/90 hover:border-[#00A3AD] transition-all duration-300 flex flex-col overflow-hidden shadow-[0_4px_20px_rgba(10,37,64,0.04)] hover:shadow-[0_12px_35px_rgba(0,163,173,0.12)] sm:hover:-translate-y-0.5">
              ...
              <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
                  <button type="button" onClick={() => setActivePreviewTool(tool)} className="text-xs font-bold text-slate-500 hover:text-[#00A3AD] transition-colors flex items-center gap-1 cursor-pointer min-h-10">
        ...
        {activePreviewTool && (
          <div id="tool-preview-modal-overlay" className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[#0A2540]/60 backdrop-blur-sm overflow-y-auto" onClick={() => setActivePreviewTool(null)}>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }} transition={{ duration: 0.2 }} className="relative w-full max-w-2xl max-h-[92dvh] overflow-y-auto overscroll-contain rounded-t-3xl sm:rounded-3xl bg-white border border-slate-200 shadow-2xl p-5 sm:p-8 text-[#0A2540]" onClick={(e) => e.stopPropagation()}>
              <button id="close-tool-preview-btn" onClick={() => setActivePreviewTool(null)} className="absolute top-3 right-3 p-2 rounded-full text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-colors z-20 cursor-pointer min-w-10 min-h-10" aria-label="Close"><X className="w-5 h-5" /></button>
              <div className="flex items-center gap-2 mb-2 pr-10">
                ...
                <span className="text-xs text-slate-500 font-mono font-bold truncate">{activePreviewTool.category}</span>
              </div>
              <h2 className="text-xl sm:text-3xl font-black text-[#0A2540] pr-8">{activePreviewTool.name}</h2>
              <div className="mt-4 rounded-2xl sm:rounded-3xl overflow-hidden border border-slate-200 bg-slate-50 shadow-inner">
                <img src={activePreviewTool.image} alt={activePreviewTool.screenshotAlt || activePreviewTool.name} referrerPolicy="no-referrer" className="w-full h-44 sm:h-60 object-cover object-center" />
              </div>
              <div className="mt-5">
              ...
              <div className="mt-5">
                ...
                <div className="flex flex-wrap gap-2">{activePreviewTool.highlights.map((h, i) => (<div key={i} className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-slate-50 border border-slate-200 text-xs text-[#0A2540] font-bold"><CheckCircle2 className="w-3.5 h-3.5 text-[#00A3AD] shrink-0" /><span>{h}</span></div>))}</div>
              </div>
              <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3">
                <button type="button" onClick={() => setActivePreviewTool(null)} className="px-5 py-3 rounded-full text-sm font-bold text-slate-500 hover:text-slate-900 transition-colors cursor-pointer min-h-11">Close</button>
