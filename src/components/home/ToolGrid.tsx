...
export const ToolGrid: React.FC<ToolGridProps> = ({ searchQuery, selectedCategory }) => {
  ...
  const getToolIcon = (iconName: string) => {
    ...
    switch (iconName) {
      case 'FileText': return <FileText {...iconProps} className="w-4 h-4 text-[#EA580C]" />;
      case 'Layers': return <Layers {...iconProps} className="w-4 h-4 text-[#7C3AED]" />;
      case 'Sparkles': return <Sparkles {...iconProps} className="w-4 h-4 text-[#EA580C]" />;
      case 'RefreshCw': return <RefreshCw {...iconProps} className="w-4 h-4 text-emerald-600" />;
      case 'Minimize2': return <Minimize2 {...iconProps} className="w-4 h-4 text-[#EA580C]" />;
      case 'Coins': return <Coins {...iconProps} className="w-4 h-4 text-[#7C3AED]" />;
      case 'Terminal': return <Terminal {...iconProps} className="w-4 h-4 text-[#C2410C]" />;
      case 'QrCode': return <QrCode {...iconProps} className="w-4 h-4 text-[#7C3AED]" />;
      ...
      case 'FileEdit': return <FileEdit {...iconProps} className="w-4 h-4 text-[#EA580C]" />;
      case 'Code2': return <Code2 {...iconProps} className="w-4 h-4 text-indigo-600" />;
      default: return <Sparkles {...iconProps} className="w-4 h-4 text-[#EA580C]" />;
    }
  };
    ...
    <section id="tools-grid" className="py-8 sm:py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 min-w-0">
      <div className="mb-6 sm:mb-8 pb-4 border-b border-slate-200/80">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-black uppercase tracking-wider text-[#EA580C]">Tools</h2>
        </div>
          ...
          {filteredTools.map((tool, index) => (
            <div key={tool.id} id={`tool-card-${tool.id}`} className="group rounded-3xl bg-white border border-slate-200/90 hover:border-[#EA580C] transition-all duration-300 flex flex-col overflow-hidden shadow-[0_4px_20px_rgba(28,25,23,0.04)] hover:shadow-[0_12px_35px_rgba(249,115,22,0.12)] sm:hover:-translate-y-0.5">
              <div className="relative h-44 sm:h-48 w-full overflow-hidden bg-[#FFEDD5] cursor-pointer" onClick={() => setActivePreviewTool(tool)}>
                ...
                <div className="absolute inset-0 hidden sm:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-[#1C1917]/35 backdrop-blur-[2px]">
                  <button type="button" onClick={(e) => { e.stopPropagation(); setActivePreviewTool(tool); }} className="px-4.5 py-2 rounded-full bg-white hover:bg-[#FFEDD5] text-[#1C1917] text-xs font-bold border border-slate-200 transition-all flex items-center gap-1.5 shadow-lg cursor-pointer active:scale-95">
                    <Info className="w-3.5 h-3.5 text-[#EA580C]" /><span>Quick Details & Guide</span>
                  </button>
                </div>
              </div>
              <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">{tool.highlights.map((h, i) => (<span key={i} className="text-[11px] px-3 py-1 rounded-full bg-[#FFEDD5] border border-[#FDBA74] text-[#C2410C] font-bold">{h}</span>))}</div>
                </div>
                <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
                  <button type="button" onClick={() => setActivePreviewTool(tool)} className="text-xs font-bold text-slate-600 hover:text-[#C2410C] transition-colors flex items-center gap-1 cursor-pointer min-h-10">
                    <Info className="w-3.5 h-3.5" /><span>Learn more</span>
                  </button>
                  <Link to={tool.route} id={`open-tool-btn-${tool.id}`} className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-gradient-to-r from-[#F97316] to-[#EA580C] text-white font-black text-xs shadow-md shadow-orange-500/20 transition-all active:scale-95 cursor-pointer min-h-10">
      ...
      <AnimatePresence>
        {activePreviewTool && (
          <div id="tool-preview-modal-overlay" className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[#1C1917]/60 backdrop-blur-sm overflow-y-auto" onClick={() => setActivePreviewTool(null)}>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }} transition={{ duration: 0.2 }} className="relative w-full max-w-2xl max-h-[92dvh] overflow-y-auto overscroll-contain rounded-t-3xl sm:rounded-3xl bg-white border border-slate-200 shadow-2xl p-5 sm:p-8 text-[#1C1917]" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-2 mb-2 pr-10">
                <span className="px-3 py-1 rounded-full bg-[#FFEDD5] text-[#C2410C] border border-[#FDBA74] text-xs font-mono font-bold">{activePreviewTool.badge}</span>
              ...
              {activePreviewTool.howItWorks && (
                <div className="mt-5 rounded-2xl sm:rounded-3xl bg-[#FFEDD5]/60 border border-[#FDBA74] p-4 sm:p-5">
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#C2410C] mb-3.5 flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-emerald-700" /><span>How it works in this tab</span></h4>
                  <div className="space-y-3">{activePreviewTool.howItWorks.map((step, idx) => (<div key={idx} className="flex items-start gap-3"><div className="w-6 h-6 rounded-full bg-[#EA580C] text-white text-xs font-black flex items-center justify-center shrink-0 mt-0.5 font-mono">{idx + 1}</div><p className="text-sm text-[#1C1917] leading-relaxed font-bold">{step}</p></div>))}</div>
                </div>
              )}
              <div className="mt-5">
                <h4 className="text-xs font-black uppercase tracking-wider text-slate-600 mb-2">Key capabilities</h4>
                <div className="flex flex-wrap gap-2">{activePreviewTool.highlights.map((h, i) => (<div key={i} className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[#FFEDD5] border border-[#FDBA74] text-xs text-[#C2410C] font-bold"><CheckCircle2 className="w-3.5 h-3.5 text-[#C2410C] shrink-0" /><span>{h}</span></div>))}</div>
              </div>
              <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3">
                ...
                <Link to={activePreviewTool.route} id="modal-launch-tool-btn" onClick={() => setActivePreviewTool(null)} className="px-6 py-3 rounded-full bg-gradient-to-r from-[#F97316] to-[#EA580C] text-white font-black text-sm shadow-lg shadow-orange-500/25 transition-all flex items-center justify-center gap-2 active:scale-95 cursor-pointer min-h-11">
