    ...
    <section className="relative pt-8 pb-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto z-10 min-w-0">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 min-w-0">
        <div className="max-w-2xl min-w-0 w-full">
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="inline-flex max-w-full items-center gap-2 px-3 py-1.5 rounded-full bg-[#E6F8F9] border border-[#B3EAEF] text-[11px] sm:text-xs font-black text-[#007A82] mb-4">
            <ShieldCheck className="w-3.5 h-3.5 text-[#00A3AD] shrink-0" />
            <span className="truncate">Free tools · files stay in this tab</span>
          </motion.div>
          <motion.h1
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-[1.65rem] sm:text-4xl lg:text-5xl font-black text-[#0A2540] tracking-tight leading-[1.25] text-pretty break-words"
          >
          ...
          <motion.p
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-3.5 text-sm sm:text-lg text-slate-600 max-w-xl font-medium leading-relaxed text-pretty break-words"
          >
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col gap-2.5 w-full lg:w-auto shrink-0">
          <a href="#tools-grid" onClick={() => track('click')} className="px-6 py-3 rounded-full bg-[#00A3AD] text-white font-bold text-sm flex items-center justify-center gap-2 min-h-11">
            <Layers className="w-4 h-4" /> {ctaLabel}
          </a>
          <Link to="/about" className="px-6 py-3 rounded-full bg-white text-[#0A2540] font-bold text-sm border border-slate-200 text-center min-h-11 flex items-center justify-center">How it works</Link>
        </div>
      </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap min-h-9 ${
                selectedCategory === cat ? 'bg-[#00A3AD] text-white' : 'bg-[#F0F6F9] text-slate-700'
              }`}
            >
              {cat}
            </button>
