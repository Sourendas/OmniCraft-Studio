...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full min-w-0 border-b border-slate-200/80 bg-white/90 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between gap-2 min-w-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          <button onClick={() => setCommandPaletteOpen(true)} className="flex items-center justify-center sm:gap-2 w-10 h-10 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 rounded-full bg-[#F4F8FA] border border-slate-200 text-xs font-bold" aria-label="Search tools">
            <Search className="w-4 h-4 text-[#00A3AD]" />
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 rounded-2xl text-slate-600 min-w-10 min-h-10 flex items-center justify-center" aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'}>{mobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button>
        </div>
      </div>
      {mobileMenuOpen && (
        <div className="lg:hidden border-b border-slate-200 bg-white p-4 space-y-3 max-h-[min(80vh,32rem)] overflow-y-auto overscroll-contain">
          <div className="grid grid-cols-1 gap-1.5">
            {TOOLS_DATA.map((t) => (
              <Link key={t.id} to={t.route} onClick={() => setMobileMenuOpen(false)} className="p-3 rounded-2xl bg-slate-50 border border-slate-200 text-sm font-bold">{t.name}</Link>
            ))}
          </div>
